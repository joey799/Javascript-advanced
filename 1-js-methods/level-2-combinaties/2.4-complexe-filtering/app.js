...
const middelsteGetallen = getallen.filter(checkGetalen) 

function checkGetalen(getal) {
    return getal > 10 && getal < 20
}
...
const langeAwoorden = woorden.filter(checkLangeAwoorden)

function checkLangeAwoorden(woord) {
    return woord.startsWith('a') && woord.length > 5
}

console.log('Lange woorden met a:', langeAwoorden);

// TODO: Filter even getallen die groter zijn dan 15
const groteEvenGetallen = getallen.filter(checkGroteEvenGetallen)
function checkGroteEvenGetallen(getal) {
    return getal > 15 && getal % 2 === 0
}
